...
public class WPMonitoring extends Thread {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 
		 * try { xmlPath = args[0]; //argumento 0 para escolher o path do xml file
		 * 
		 * ObjectMapper mapper = new XmlMapper(); InputStream inputStream = new
		 * FileInputStream(new File(xmlPath)); TypeReference<List<TipoTeste>>
		 * typeReference = new TypeReference<List<TipoTeste>>() { }; List<TipoTeste>
		 * testes = mapper.readValue(inputStream, typeReference); for (TipoTeste p :
		 * testes) {
		 * 
		 * testesLista.add(p);
		 * 
		 * //System.out.println("Page " + p.getPage() + " city is " +
		 * p.getAddress().getCity()+ " first car is " + p.getCars()[0] + " age is " +
		 * p.getAge());
		 * 
		 * }
		 * 
		 ...
		 * println("Ficheiro XML em falta. Exemplo: java -jar <ficheiro.jar> <c:\\config.xml>"
		 * ); System.exit(1); }
		 */
